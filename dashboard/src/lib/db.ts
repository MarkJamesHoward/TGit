...
const databaseId = process.env.COSMOS_DATABASE || "tgit-dashboard";
...
export async function getUser(id: string): Promise<PasskeyUser | null> {
  await ensureInitialized();
  try {
    const { resource } = await container("passkey_users")
      .item(id, id)
      .read<PasskeyUser>();
...
export async function getUserByDisplayName(
  ...
  await ensureInitialized();
  const { resources } = await container("passkey_users")
    .items.query({
      query: "SELECT * FROM c WHERE c.displayName = @name",
      parameters: [{ name: "@name", value: displayName }],
    })
    .fetchAll();
...
export async function updateUserLastTenant(
  ...
  if (user) {
    await container("passkey_users")
      .item(userId, userId)
      .replace({ ...user, lastTenant: tenant });
  }
}
...
export async function getCredentialsByUserId(
  ...
  await ensureInitialized();
  const { resources } = await container("passkey_credentials")
    .items.query({
...
export async function getCredentialByCredentialId(
  ...
  const { resources } = await container("passkey_credentials")
    .items.query({
      query: "SELECT * FROM c WHERE c.credentialId = @cid",
      parameters: [{ name: "@cid", value: credentialId }],
    })
    .fetchAll();
  return resources[0] || null;
}
...
export async function updateCredentialCounter(
  ...
  if (cred) {
    await container("passkey_credentials")
      .item(cred.id, cred.userId)
      .replace({ ...cred, counter });
  }
}
...
export async function getSession(id: string): Promise<PasskeySession | null> {
  await ensureInitialized();
  try {
    const { resource } = await container("passkey_sessions")
      .item(id, id)
      .read<PasskeySession>();
...
export async function getChallenge(id: string): Promise<string | null> {
  await ensureInitialized();
  try {
    const { resource } = await container("passkey_challenges")
      .item(id, id)
      .read<any>();
    if (!resource) return null;
